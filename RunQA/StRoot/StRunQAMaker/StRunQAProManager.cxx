...
void StRunQAProManager::initRunQA()
{
  for(int i_cut = 0; i_cut < 2; ++i_cut)
  {
    ...
    p_mQA_Vz[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_Vr_%s",mCutsQA[i_cut].c_str());
    p_mQA_Vr[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_gDca_%s",mCutsQA[i_cut].c_str());
    p_mQA_gDca[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_nHitsFit_%s",mCutsQA[i_cut].c_str());
    p_mQA_nHitsFit[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_PrimPt_%s",mCutsQA[i_cut].c_str());
    p_mQA_PrimPt[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_PrimEta_%s",mCutsQA[i_cut].c_str());
    p_mQA_PrimEta[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_PrimPhi_%s",mCutsQA[i_cut].c_str());
    p_mQA_PrimPhi[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_GlobPt_%s",mCutsQA[i_cut].c_str());
    p_mQA_GlobPt[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_GlobEta_%s",mCutsQA[i_cut].c_str());
    p_mQA_GlobEta[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);

    ProName = Form("p_mQA_GlobPhi_%s",mCutsQA[i_cut].c_str());
    p_mQA_GlobPhi[i_cut] = new TProfile(ProName.c_str(),ProName.c_str(),runQA::mNumOfRunIndex,-0.5,(float)runQA::mNumOfRunIndex-0.5);
  }
}
...
void StRunQAProManager::fillRunQA_Track(int runIdenx, float gDca, int nHitsFit, TVector3 pMom, TVector3 gMom, int cutSelection)
{
