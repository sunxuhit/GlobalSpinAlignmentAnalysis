...
then
  ...
  get_file_list.pl -keys 'path,filename' -cond 'production=P19ie,library=SL20d,trgsetupname=production_3p85GeV_fixedTarget_2018,collision=AuAu8,filetype=daq_reco_picoDst,filename~st_physics,storage!=hpss' -limit 0 -delim / -distinct > $OutPutDir/pico.list
