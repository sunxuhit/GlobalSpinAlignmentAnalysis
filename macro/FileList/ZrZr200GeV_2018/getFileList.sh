...
then
  ...
  get_file_list.pl -keys 'path,filename' -cond 'production=P20ic,library=SL20c,trgsetupname=production_isobar_2018,collision=ZrZr200,filetype=daq_reco_picoDst,filename~st_physics,storage!=hpss' -limit 0 -delim / -distinct > $OutPutDir/pico.list
